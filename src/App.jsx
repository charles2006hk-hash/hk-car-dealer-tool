...
const PrintableReport = ({ data, onClose, logo }) => {
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col items-center p-4 print:p-0 print:bg-white">
            <style>{`
                @media print {
                    @page { size: A4; margin: 10mm; }
                    body { background: white; }
                    .no-print { display: none !important; }
                    .print-content { width: 100% !important; margin: 0 !important; box-shadow: none !important; }
                }
            `}</style>

            {/* Print Toolbar */}
            <div className="w-full max-w-[210mm] flex justify-between items-center mb-6 no-print">
                <button onClick={onClose} className="flex items-center gap-2 px-4 py-2 bg-white text-slate-700 rounded-lg shadow font-bold hover:bg-slate-50"><ArrowLeft className="w-5 h-5"/> 返回計算器</button>
                <button onClick={handlePrint} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg shadow font-bold hover:bg-blue-700"><Printer className="w-5 h-5"/> 列印 / PDF</button>
            </div>

            {/* Report Content - A4 Width */}
            <div className="print-content w-full max-w-[210mm] bg-white shadow-2xl p-10 min-h-[297mm] flex flex-col relative">
                {/* Header */}
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState('calculator'); // 'calculator', 'history', 'settings', 'report_view'
  ...
  const generateCurrentReport = () => {
      ...
      setActiveTab('report_view'); // Switch logical tab (handled by conditional render)
  };
  ...
  const closeReport = () => {
      setReportData(null);
      // Don't change activeTab here, user might want to stay on history
  };
  ...
  // Render logic: If reportData is present, show report view (which handles printing)
  if (reportData) {
