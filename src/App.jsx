...
const InputGroup = ({ label, value, onChange, prefix, placeholder = "", required = false }) => (
  <div className="mb-3">
    ...
    <div className="relative rounded-md shadow-sm">
      ...
      <input type="number" step="any" className={`focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md py-2 ${prefix ? 'pl-8' : 'pl-3'}`} placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)} />
    </div>
  </div>
);
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <Card className="max-w-sm w-full animate-in fade-in zoom-in-95">
          <div className={`p-4 border-b ${type === 'danger' ? 'bg-red-50 border-red-100 text-red-800' : 'bg-yellow-50 border-yellow-100 text-yellow-800'}`}><h3 className="font-bold flex gap-2"><AlertTriangle className="w-5 h-5" />{title}</h3></div>
          ...
          <div className="flex justify-end gap-2 p-4 border-t bg-gray-50">
            ...
            <button onClick={onConfirm} className={`px-3 py-1.5 text-white rounded text-sm ${type === 'danger' ? 'bg-red-600 hover:bg-red-700' : 'bg-yellow-600 hover:bg-yellow-700'}`}>確認</button>
          </div>
        </Card>
      </div>
    );
                           ...
                           {Object.keys(inventory).map(mfr => (
                               <div key={mfr} className="border rounded-lg bg-gray-50 overflow-hidden">
                                   <div className="flex justify-between items-center p-3 cursor-pointer hover:bg-gray-100" onClick={() => setEditingMfr(editingMfr === mfr ? null : mfr)}>
                                       <span className="font-bold text-sm">{mfr} ({inventory[mfr].models.length})</span>
                                   ...
                                   {editingMfr === mfr && (
                                       <div className="p-3 border-t bg-white">
                                           ...
                                           {inventory[mfr].models.length === 0 && <div className="text-xs text-gray-400 text-center">無型號</div>}
                                       </div>
                                   )}
                               </div>
                           ))}
                   ...
                   <Card className="p-4">
                      ...
                      <div className="grid grid-cols-3 gap-3">
                          {Object.keys(DEFAULT_RATES).map(c => (
                              <InputGroup key={c} label={c} value={rates[c]} onChange={v => setRates(r => ({...r, [c]: v}))} />
                          ))}
                      </div>
                   </Card>
